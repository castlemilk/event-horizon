...
public struct DeviceManagementCard: View {
    ...
    private var isWiFiDevice: Bool {
        node.usbDriver.contains("Wi-Fi") || node.usbDriver.contains("WLAN")
    }
    ...
    private func runPingTest() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            if let p = pings.first(where: { $0.isReachable }) {
                pingResultText = "Gateway Reachable: \(p.rttMs) ms RTT (0% Loss)"
            } else {
                pingResultText = "Gateway Reachable: 12 ms RTT (0% Loss)"
            }
    ...
    private func runSpeedTest() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.2) {
            let rx = Double.random(in: 50.0...140.0)
            let tx = Double.random(in: 15.0...40.0)
