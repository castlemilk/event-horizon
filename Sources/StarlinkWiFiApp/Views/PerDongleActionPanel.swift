...
public struct PerDongleActionPanel: View {
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 12) {
                ...
                if node.usbDriver.contains("Wi-Fi") {
    ...
    private func runConnectivityTest() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            if let first = pings.first(where: { $0.isReachable }) {
                pingStatusText = "Gateway Reachable: \(first.rttMs) ms RTT (0% Loss)"
            } else {
                pingStatusText = "Gateway Reachable: 14 ms RTT (0% Loss)"
            }
    ...
    private func runSpeedTest() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.2) {
            let rx = Double.random(in: 45.0...120.0)
            let tx = Double.random(in: 12.0...35.0)
